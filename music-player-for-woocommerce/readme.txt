...
* Modifies the plugin settings page.

= 1.1.8 =

* Checks if the woocommerce_init action was triggered before displaying the players in the titles of the products.

= 1.1.7 =

* Modifies the module that identifies the local files.

= 1.1.6 =

* Includes a new column in the products list with the playback counter.
...
* Modifies access to the resources files.
* Fixes an issue when modifying the time interval to play.
* Modifies the editor integration.
* Improves the generation of audio files for demos in the Professional version of the plugin.

= 1.1.2 =

* Fixes an issue with classic layouts.
* Applies overflow hidden to the players' containers.
...
* Implements the 'Show a single player instead of one player per audio file' option in the players' settings, to display only one player with the associated playlist on products with multiple audio files instead of a separate player per file.
